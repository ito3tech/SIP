...
def get_image_arr( path , width , height , imgNorm="sub_mean" , odering='channels_first' ):
	...
	if type( path ) is np.ndarray:img = path
	else:img = cv2.imread(path, 1)
	...
	#add gray channel	
	gr_img = np.clip((alpha*255*(img/255)**gamma + beta),0,255).astype("uint8")
	#gr_img = cv2.fastNlMeansDenoisingColored(gr_img,None,12,12,1,10)
	gr_img = cv2.cvtColor(gr_img,cv2.COLOR_RGB2GRAY)
	gr_img = np.expand_dims(gr_img,axis =2)

	total_im = np.concatenate((img,gr_img),axis=2)
	if odering == 'channels_first':
		total_im = np.rollaxis(total_im,2,0)
	#del img,gr_img
	#gc.collect()
	return total_im
	#return img
