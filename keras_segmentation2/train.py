	...
	if not validate:
		for ep in range( epochs ):
			...
			model.fit_generator( train_gen , steps_per_epoch  , epochs=1 ,callbacks=callbacks,
                                             use_multiprocessing=True, workers = process_count,max_queue_size = process_count *10)
			if not checkpoints_path is None:
		...
		for ep in range( epochs ):
			print("Starting Epoch " , ep )
			model.fit_generator( train_gen , steps_per_epoch  , validation_data=val_gen , validation_steps=50 ,
